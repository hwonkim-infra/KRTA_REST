...
const HEXCalc = (values) => {
    ...
    const ground_Length =
        0.65 * values.undercarriage.tumbler_distance + 0.35 * values.undercarriage.track_length; // 접지길이
    const ground_pressure = roundTwo(grossWeight / ((((2 * values.undercarriage.shoe_width) / 10) * ground_Length) / 10)) || null; // 접지압
    const ground_pressure_woqc = roundTwo((grossWeight - quick_coupler_weight) / ((((2 * values.undercarriage.shoe_width) / 10) * ground_Length) / 10)) || null; // 접지압
    ...
    const travel_speed = roundTwo(Math.max(travel_speed_1, travel_speed_2)) || null;
    ...
    const traction_downward = (values.travel.sprocket_radius / 1000 * grossWeight * Math.sin(16.7 / 180 * Math.PI));

    const theta_1 = (Math.atan(values.travel.surface_drag) * (180 / Math.PI)).toFixed(1);
    const theta_2 = ((180 / Math.PI) * Math.asin((DP - values.travel.drag * grossWeight) / grossWeight)).toFixed(1);
