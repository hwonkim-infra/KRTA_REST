const roundTwo = (num) => {return +(Math.round(num + "e+2") + "e-2")};

const roundOne = (num) => {return +(Math.round(num + "e+1") + "e-1")};
...
// console.log("🚀 ~ file: WEXCalc.jsx ~ line 11 ~ WEXCalc ~ values", values)
    ...
    const quick_coupler_weight = Math.max(  // 퀵 커플러 중량
    ...
    /* 주행성능 */
    const rearAxle_center = values.undercarriage.wheel_base - values.undercarriage.frontAxle_center;
const axle_weight_front_unload = Number(values.operating_weight * (rearAxle_center-values.undercarriage.COG_center_unload)/values.undercarriage.wheel_base) || ''; // 공차하중 1축
const axle_weight_rear_unload = Number(values.operating_weight * (values.undercarriage.frontAxle_center+values.undercarriage.COG_center_unload)/values.undercarriage.wheel_base) || ''; // 공차하중 2축

// 적재하중 1,2축
const axle_weight_front_load = Number(grossWeight_load * (rearAxle_center - values.undercarriage.COG_center_load)/values.undercarriage.wheel_base) || '';
const axle_weight_rear_load = Number(grossWeight_load* (values.undercarriage.frontAxle_center+values.undercarriage.COG_center_load)/values.undercarriage.wheel_base) || '';

  const axle_motor_rev=(values.travel.pump_displacement_travel *
    values.travel.motor_eff_travel/ values.travel.motor_displacement_travel* 1000).toFixed(); 
const travel_speed=(2 * Math.PI * axle_motor_rev * values.travel.tire_rolling_radius /1000 * 60 /(values.travel.TM_reduction * values.travel.axle_reduction * 10**3)).toFixed()


  const noslip_slope = (radians_to_degrees(Math.atan(values.travel.friction_surface))).toFixed(1);
const traction_slope = (radians_to_degrees(Math.asin((values.travel.traction_force - values.travel.running_resist *
    grossWeight)/grossWeight))).toFixed(1);
const min_slope = (Math.min(values.travel.engine_slope, noslip_slope, traction_slope)).toFixed();

const innerKingpin_COS = Math.round(values.undercarriage.wheel_base / Math.sin(degrees_to_radians(values.travel.wheel_angle)));
const outer_rim_minRadius = Math.round(innerKingpin_COS + values.travel.kingpin_offset);
...
const braking_force_axle	= (values.travel.brake_torque_axle/(9.8 * values.travel.tire_rolling_radius / 1000)).toFixed()
const braking_force_total	= (2*braking_force_axle).toFixed()
const braking_limit_slope	= (radians_to_degrees(Math.asin(braking_force_total/grossWeight))).toFixed(1)
...
const braking_distance_max = (travel_speed**2/(2*decceleration)*((1000/3600)**2) + idle_running*travel_speed*(1000/3600)).toFixed(1);
const braking_distance_norm = (braking_speed_standard**2/(2*decceleration)*(1000/3600)**2 + idle_running*braking_speed_standard*(1000/3600)).toFixed(1);
